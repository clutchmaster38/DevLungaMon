...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion && Input.get_mouse_mode() == 2:
		...
		var linePoint2 = get_viewport().get_camera_3d().unproject_position((get_node("/root/TestWorld/Player").global_position) + Vector3(0,0,100).rotated(Vector3(1,0,0), sunPoint))
...
func _physics_process(delta):
	...
	var query = PhysicsRayQueryParameters3D.create($Camera3D.global_position,(get_node("/root/TestWorld/Player").global_position) + Vector3(0,0,100).rotated(Vector3(1,0,0), sunPoint))
